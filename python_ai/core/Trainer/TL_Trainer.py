...
class TL_Trainer(ModelTrainer):
    def __init__(self, cfg, from_config=True, **kwargs):
        ...
        self.learning_rate = cfg.get('TRANSFER_LEARNING.LEARNING_RATE')
        # self.epochs = cfg.TRANSFER_LEARNING.EPOCH
        self.epochs = cfg.get('TRANSFER_LEARNING.EPOCH')
        self._setup_logger()
        self._setup_metrics()
        ...
        self.beta = cfg.get('TRANSFER_LEARNING.BETA') if from_config else kwargs['beta']
        self.training_anomaly = kwargs['training_anomaly']
        # self.batch_size = cfg.TRANSFER_LEARNING.ANOM_BATCH_SIZE \
        self.batch_size = cfg.get('TRANSFER_LEARNING.ANOM_BATCH_SIZE') \
