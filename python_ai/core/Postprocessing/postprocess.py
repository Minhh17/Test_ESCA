...
class Postprocessor():
    ...
    def _log_histogram(self, losses):
        """
        Log loss histogram information.
        Prefix: [HIST]
        JSON data: { "value": <total_loss> }
        """

        # print(f'[HIST]{{"value": {losses}}}', flush=True)
        print(f'[HIST] {",".join(map(str, losses))}', flush=True)

    def _log_pr_curve(self, predictions, labels):
        """
        Log PR curve representative values.
        Prefix: [PR]
        JSON data: { "recall": x, "precision": y }
        """
        # recall, precision = self.post_prc.calculate_pr_curve(predictions, labels)
        precision, recall, thresholds = precision_recall_curve(labels, predictions)

        json_data = json.dumps([recall.tolist(), precision.tolist()], separators=(",", ":"))  # Chuyển sang list
        print(f"[PR]{json_data}", flush=True)


    def _log_roc_curve(self, predictions, labels):
        """
        Log ROC curve representative values.
        Prefix: [ROC]
        JSON data: { "fpr": x, "tpr": y }
        """
        # fpr, tpr = self.post_prc.calculate_roc_curve(predictions, labels)
        fpr, tpr, _ = roc_curve(labels, predictions)
        json_data = json.dumps([fpr.tolist(), tpr.tolist()], separators=(",", ":"))
        print(f"[ROC]{json_data}", flush=True)
        # print(f'[ROC]{{"fpr": {fpr}, "tpr": {tpr}}}', flush=True)
